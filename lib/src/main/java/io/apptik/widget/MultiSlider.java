...
public class MultiSlider extends View {
	...
	public interface OnThumbValueChangeListener {
		 ...
		 */
		void onValueChanged(MultiSlider multiSlider, MultiSlider.Thumb thumb, int thumbIndex, double
				value);
		 ...
		 */
		void onStartTrackingTouch(MultiSlider multiSlider, MultiSlider.Thumb thumb, double value);
		 ...
		 */
		void onStopTrackingTouch(MultiSlider multiSlider, MultiSlider.Thumb thumb, double value);
	}
	 ...
	 * global Min and Max
	 */
	private double mScaleMin;
	private double mScaleMax;
	...
	public class Thumb {
		 ...
		 * @return
		 */
		public Thumb setMin(double min) {
			if (min > this.max) {
				min = this.max;
			}
			if (min < mScaleMin) {
		 ...
		 * @return
		 */
		public Thumb setMax(double max) {
			if (max < this.min) {
				max = this.min;
			}
			if (max > mScaleMax) {
		 ...
		 * @return
		 */
		public Thumb setValue(double value) {
...
//        mMaxHeight = a.getDimensionPixelSize(R.styleable.MultiSlider_maxHeight, mMaxHeight);
		...
		setStep(a.getFloat(io.apptik.widget.R.styleable.MultiSlider_scaleStep, (float) mStep));
		...
		setDrawThumbsApart(a.getBoolean(io.apptik.widget.R.styleable.MultiSlider_drawThumbsApart,
				mDrawThumbsApart));
		setMax(a.getFloat(io.apptik.widget.R.styleable.MultiSlider_scaleMax, (float) mScaleMax), true);
		setMin(a.getFloat(io.apptik.widget.R.styleable.MultiSlider_scaleMin, (float) mScaleMin), true);
	 ...
	 */
	public void setStep(double mStep) {
		this.mStep = mStep;
	}
	...
	private void positionThumbs() {
		...
		if (mThumbs.size() > 2) {
			double even = (mScaleMax - mScaleMin) / (mThumbs.size() - 1);
			double lastPos = mScaleMax - even;
...
//            }
	...
	private double optThumbValue(Thumb thumb, double value) {
	 ...
	 */
	private synchronized void setThumbValue(Thumb thumb, double value, boolean fromUser) {
		if (thumb == null || thumb.getThumb() == null) {
			return;
		}
	...
	private synchronized void setThumbValue(int thumb, double value, boolean fromUser) {
	 ...
	 */
	public synchronized void setMax(double max, boolean extendMaxForThumbs) {
	 ...
	 */
	public synchronized void setMax(double max, boolean extendMaxForThumbs, boolean repositionThumbs) {
		if (max < mScaleMin) {
			max = mScaleMin;
		}
	 ...
	 */
	public synchronized void setMin(double min, boolean extendMinForThumbs) {
	 ...
	 *                           equal distances from each other respecting the new scale
	 */
	public synchronized void setMin(double min, boolean extendMinForThumbs, boolean repositionThumbs) {
		if (min > mScaleMax) {
			min = mScaleMax;
		}
	...
	private Thumb getMostMovable(LinkedList<Thumb> thumbs, MotionEvent event) {
		...
		if (thumbs != null && !thumbs.isEmpty()) {
			...
			for (Thumb thumb : thumbs) {
				double optValue = (getValue(event, thumbs.getFirst()) > thumb.getValue()) ?
						mScaleMax : mScaleMin;
				double currChange = Math.abs(thumb.getValue() - optThumbValue(thumb, optValue));
		...
		switch (event.getActionMasked()) {
			case MotionEvent.ACTION_DOWN:
				if (isInScrollingContainer()) {
					...
					double value = getValue(event, currThumb);
...
//                    onStopTrackingTouch();
	...
	private double getValue(MotionEvent event, Thumb thumb) {
	...
	private double getValue(MotionEvent event, int pointerIndex, Thumb thumb) {
...
//    }
	...
	public static class SimpleOnThumbValueChangeListener implements OnThumbValueChangeListener {
		...
		@Override
		public void onValueChanged(MultiSlider multiSlider, Thumb thumb, int thumbIndex, double
				value) {
		}

		@Override
		public void onStartTrackingTouch(MultiSlider multiSlider, Thumb thumb, double value) {
		}

		@Override
		public void onStopTrackingTouch(MultiSlider multiSlider, Thumb thumb, double value) {
		}
